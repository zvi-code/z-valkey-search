...
#ifndef VMSDK_SRC_VALKEYMODULE_H
#define VMSDK_SRC_VALKEYMODULE_H
...
#define VALKEYMODULE_EVENT_CLUSTER_STATE_CHANGED 18
...
/* Forward declaration for use in ValkeyModule_Init */
static int ValkeyModule_Init_G(void);
...
static int ValkeyModule_Init(ValkeyModuleCtx *ctx, const char *name, int ver, int apiver) {
    ...
    if (ValkeyModule_Init_G() != VALKEYMODULE_OK) {
...
/* Google-internal module APIs are imported at the end, so they have access to
 * the symbols defined in this file. */
#include "valkeymodule_g.h"

#endif /* VMSDK_SRC_VALKEYMODULE_H */
