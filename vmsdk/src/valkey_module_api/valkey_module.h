...
static const RedisModuleEvent
    ...
    RedisModuleEvent_ClusterStateChanged = {
        REDISMODULE_EVENT_CLUSTER_STATE_CHANGED,
        1
    };
...
typedef struct {
    uint64_t psc_connection_id;      /* PSC connection ID to uniquely identify a PSC connection */
    const char *psc_port_mapping_ip; /* PSC port mapping IP address */
    const char *psc_ilb_ip;          /* PSC ILB IP address */
    int is_internal;                 /* Flag to indicate if the connection is from an internal source */
} RedisModuleConnectionProperty;

/* flags on the purpose of module rdb save or load */
#define RDB_STREAM_FLAG_NONE 0
#define RDB_STREAM_FLAG_KEEP_EXISTING_DATA (1<<20)  /* Keep existing data during load */
#define RDB_STREAM_FLAG_USE_RDB_COMPRESSION (1<<21) /* Use RDB compression when saving. */

/* Version of the RedisModuleRIOHandler structure. Once the RedisModuleRIOHandler
 * structure is changed, this version number needs to be changed synchronistically. */
...
REDISMODULE_API int (*RedisModule_SetConnectionProperties)(const RedisModuleConnectionProperty *properties, int len) REDISMODULE_ATTR;
...
REDISMODULE_API void (*RedisModule_SetModuleExtendedOptions)(RedisModuleCtx *ctx, int options) REDISMODULE_ATTR;
...
REDISMODULE_API RedisModuleCrossClusterReplicasList * (*RedisModule_GetCrossClusterReplicasList)(void) REDISMODULE_ATTR;
REDISMODULE_API void (*RedisModule_FreeCrossClusterReplicasList)(RedisModuleCrossClusterReplicasList* head) REDISMODULE_ATTR;
...
REDISMODULE_API int (*RedisModule_SetShardId)(const char *shardId, int len) REDISMODULE_ATTR;
...
REDISMODULE_API RedisModuleRdbStream *(*RedisModule_RdbStreamCreateFromRioHandler)(const RedisModuleRIOHandler *handler) REDISMODULE_ATTR;
...
REDISMODULE_API int (*RedisModule_ReplicationSetMasterCrossCluster)(RedisModuleCtx *ctx, const char *ip, const int port) REDISMODULE_ATTR;
REDISMODULE_API int (*RedisModule_ReplicationUnsetMasterCrossCluster)(RedisModuleCtx *ctx) REDISMODULE_ATTR;
REDISMODULE_API int (*RedisModule_ReplicationSetSecondaryCluster)(RedisModuleCtx *ctx, bool is_secondary_cluster) REDISMODULE_ATTR;
REDISMODULE_API int (*RedisModule_HashExternalize)(RedisModuleKey *key, RedisModuleString *field, RedisModuleHashExternCB fn, void *privdata) REDISMODULE_ATTR;
