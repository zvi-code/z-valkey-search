...
class TestRDBCorruptedIndex(ValkeySearchTestCaseBase):
    ...
    def _start_server(self, port, test_name, rdbfile="dump.rdb", search_module_args=""):
        ...
        if os.path.exists(rdb_path_in_testdir):
            logging.info(f"RDB file exists at: {rdb_path_in_testdir}, size: {os.path.getsize(rdb_path_in_testdir)} bytes")
        else:
    ...
    def test_corrupted_rdb_load_fails(self):
        ...
        try:
            server, client, logfile = self._start_server(
                self.get_bind_port(),
                "corrupted_rdb_test_fail", 
                self.rdb_path
            )
            
            # If we get here, the server started (which shouldn't happen)
            logging.error("Server started successfully but should have failed with corrupted RDB")
                    ...
                    for logfile in os.listdir(test_dir):
                        if logfile.startswith("logfile_"):
                            ...
                            with open(log_path, 'r') as f:
                                log_content = f.read()
                                logging.info(f"Server log content from {log_path}:\n{log_content}")
                                log_found = True
                                
                                # Verify the failure was due to RDB corruption
                                if "Failed to load ValkeySearch aux section from RDB" in log_content:
                                    logging.info("✅ Confirmed: Server failed due to corrupted search index in RDB")
                                    return  # Test passed
                                elif "Short read or OOM loading DB" in log_content:
                                    logging.info("✅ Confirmed: Server failed due to RDB corruption")
                                    return  # Test passed
                                elif "Internal error in RDB reading" in log_content:
                                    logging.info("✅ Confirmed: Server failed due to RDB reading error")
                                    return  # Test passed
    ...
    def test_corrupted_rdb_skip_index_load_succeeds(self):
        ...
        try:
            ...
            assert b"index_to_test_skip_load" in indices, "Index not found in index list"
            ...
            assert expected_fields.issubset(field_names), f"Missing fields. Expected: {expected_fields}, Got: {field_names}"
                ...
                if isinstance(attr, list):
                    ...
                    if attr_dict.get(b'identifier') == b'embedding':
                        ...
                        if isinstance(index_params, list):
                            index_dict = {index_params[i]: index_params[i+1] for i in range(0, len(index_params), 2)}
                            assert b'dimensions' in index_dict or b'DIM' in index_dict
                            assert b'distance_metric' in index_dict or b'algorithm' in index_dict or b'ALGORITHM' in index_dict
                            # Verify it's using HNSW algorithm
                            algorithm = index_dict.get(b'algorithm') or index_dict.get(b'ALGORITHM')
                            if isinstance(algorithm, list) and len(algorithm) >= 2:
                                # Algorithm is stored as [b'name', b'HNSW', ...other params...]
                                assert algorithm[0] == b'name', f"Expected algorithm name field, got {algorithm[0]}"
                                assert algorithm[1] == b'HNSW', f"Expected HNSW algorithm, got {algorithm[1]}"
                            else:
                                assert algorithm == b'HNSW', f"Expected HNSW algorithm, got {algorithm}"
            ...
            while time.time() - start_time < max_wait_time:
                ...
                logging.info(f"Waiting for mutations to process, queue size: {mutation_queue_size}")
                time.sleep(0.5)
            ...
            assert num_docs > 0, f"Expected documents to be indexed after backfill, got {num_docs}"
            ...
            try:
                ...
                results = client.execute_command(
                    "FT.SEARCH", "index_to_test_skip_load",
                    "*=>[KNN 1 @embedding $vec AS score]",
                    "PARAMS", "2", "vec", embedding_hex,
                    "RETURN", "1", "score",
                    "DIALECT", "2"
                )
