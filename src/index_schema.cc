...
absl::StatusOr<std::shared_ptr<IndexSchema>> IndexSchema::LoadFromRDB(
  ...
  bool skip_loading_data = options::GetSkipIndexLoad().GetValue();
  // When skipping data, create attributes immediately (with empty indexes)
  // When loading normally, skip attributes here (they'll be loaded from RDB)
  bool skip_attributes_in_create = !skip_loading_data; 
  // Attributes will be loaded from supplemental content.
  VMSDK_ASSIGN_OR_RETURN(
      auto index_schema,
      IndexSchema::Create(ctx, *index_schema_proto, mutations_thread_pool,
                          skip_attributes_in_create));
  if (ABSL_PREDICT_FALSE(skip_loading_data)) {
