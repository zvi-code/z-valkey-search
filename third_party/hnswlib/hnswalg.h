            ...
            for (tableint i = 0; i < element_levels_.size(); i++) {
                if (element_levels_[i] > 0 &&
                    *reinterpret_cast<char**>((*linkLists_)[i]) != nullptr) {
                    free(*reinterpret_cast<char**>((*linkLists_)[i]));
                }
            }
            linkLists_->clear();
        }
...
//                bool cur_node_deleted = isMarkedDeleted(current_node_id);
            ...
            __builtin_prefetch((char *) (visited_array + *(data + 1)), 0, 3);
            __builtin_prefetch((char *) (visited_array + *(data + 1) + 64), 0, 3);
            __builtin_prefetch((*data_level0_memory_)[(*(data + 1))] + offsetData_, 0, 3);
...
//                    if (candidate_id == 0) continue;
                ...
                if (j + 1 < size) {
                    ...
                    __builtin_prefetch((*data_level0_memory_)[(*(data + j + 1))] + offsetData_,
                                    0, 3);
                }
#endif
...
#ifdef USE_PREFETCH
                        __builtin_prefetch((*data_level0_memory_)[candidate_set.top().second] +
                                        offsetLevel0_,  ///////////
                                        0, 3);  ////////////////////////
#endif
    ...
    linklistsizeint *get_linklist(tableint internal_id, int level) const {
        return (linklistsizeint *) (*reinterpret_cast<char**>((*linkLists_)[internal_id]) + (level - 1) * size_links_per_element_);
    }
    ...
    */
    ...
    tableint addPoint(const void *data_point, labeltype label, int level) {
        ...
        *data_ptr = static_cast<const char *>(data_point);
        ...
        if (curlevel) {
            *reinterpret_cast<char**>((*linkLists_)[cur_c]) = (char *) malloc(size_links_per_element_ * curlevel + 1);
            if (*reinterpret_cast<char**>((*linkLists_)[cur_c]) == nullptr)
                throw std::runtime_error("Not enough memory: addPoint failed to allocate linklist");
            memset(*reinterpret_cast<char**>((*linkLists_)[cur_c]), 0, size_links_per_element_ * curlevel + 1);
        }
    ...
    std::priority_queue<std::pair<dist_t, labeltype >>
    searchKnn(const void *query_data, size_t k, std::optional<size_t> ef_runtime, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        if (bare_bone_search) {
            top_candidates = searchBaseLayerST<true>(
                    currObj, query_data, std::max(ef_runtime.value_or(ef_), k), isIdAllowed);
        } else {
